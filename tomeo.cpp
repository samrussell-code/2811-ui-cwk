...
void toggleLikeState() {
    ...
    isLiked = !isLiked;
    likeButton->setIcon(isLiked ? likedIcon : unlikedIcon);
}
...
int main(int argc, char *argv[]) {
    ...
    likeButton = new QPushButton(videoWidget);
    likedIcon = QIcon("/path/to/liked/icon");
    unlikedIcon = QIcon("/path/to/unliked/icon");
    likeButton->setIcon(unlikedIcon);
    likeButton->setIconSize(QSize(50, 50));
    likeButton->setFlat(true);
    likeButton->setGeometry(724, 425, 50, 50);

    // Connect like button's click signal
    QObject::connect(likeButton, &QPushButton::clicked, &toggleLikeState);
